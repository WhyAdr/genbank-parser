...
REFERENCE_METADATA = {
    "kegg": ("snapshot 2026-08-02", "2026-08-10"),
    "cog": ("COG 2024", "2026-08-10"),
    "ec": ("supplied ExplorEnz XML snapshot", "2026-08-10"),
    "pfam": ("supplied Pfam-A snapshot", "2026-08-10"),
}
...
def _definition_mismatch(marker_id: str, definition: str) -> str | None:
    ...
    direct_conflicts = {
        ...
        "rhlRI": (
            r"\blasI\b",
            "LasI, not the RhlI member named by this marker",
        ),
        "sfp": (
...
def audit_markers(
    ...
    for (kind, identifier), occurrences in by_identifier.items():
        ...
        for finding in occurrences:
            ...
            if finding.audit_status == "PRESENT_UNREVIEWED":
                finding.audit_status = "DUPLICATED"

    # KO-definition ECs are compared across the complete marker. A marker may
    # represent several proteins, so additional KO ECs are broader scope, not
    # a contradiction, when at least one embedded/marker EC pair is compatible.
    for marker_id, data in marker_data.items():
        ...
        if not embedded or not marker_ecs:
            continue
        compatible = [
            (ko, ec, own)
            for ko, ec in embedded
            for own in marker_ecs
            if _ec_compatible(ec, own)
        ]
        unmatched = [
            (ko, ec)
            for ko, ec in embedded
        ...
        if compatible:
            classification = (
                ...
                if unmatched
                else "KO_EC_EXACT_COMPATIBLE"
            )
            audit_status = "PRESENT_UNREVIEWED"
            reported = unmatched or [(ko, ec) for ko, ec, _ in compatible]
            note = (
                "at least one KO EC matches; additional EC scope is broader"
                if unmatched
                else "all embedded KO ECs are compatible with marker ecs"
            )
        else:
            classification = "KO_EC_NO_COMPATIBLE_EC"
            audit_status = "REVIEW"
            reported = embedded
            note = "no embedded KO EC is compatible with marker ecs"
        reference_name = "; ".join(f"{ko}: {ec}" for ko, ec in reported)
        ...
        if not compatible:
            conflicting_kos = {ko for ko, _ in embedded}
            for finding in findings:
                if (
                    finding.marker_id == marker_id
                    and finding.identifier_type == "KO"
                    and finding.identifier in conflicting_kos
                ):
                    finding.notes = (
                        f"{finding.notes}; no compatible marker EC".lstrip("; ")
                    )
                    if finding.audit_status == "PRESENT_UNREVIEWED":
...
def write_reference_manifest(path: Path, reference_paths: dict[str, Path]) -> None:
    rows: list[list[str]] = [
        [
            "reference",
            "filename",
    ...
    for label in ("kegg", "cog", "ec", "pfam"):
        source = _require_file(reference_paths[label], f"{label} reference")
        declared_release, retrieved_date = REFERENCE_METADATA[label]
        rows.append(
            [
                label,
                source.name,
...
def write_reports(
    ...
    write_reference_manifest(outdir / "reference_manifest.tsv", reference_paths)
    _write_tsv(
    ...
    lines = [
        ...
        "## Inputs",
        "",
        f"- `markers`: `{markers_path.as_posix()}`",
    ]
    for label in ("kegg", "cog", "ec", "pfam"):
        lines.append(f"- `{label}`: `{reference_paths[label].as_posix()}`")
    lines.extend(
        [
            f"- `ec_source_used`: `{Path(references.ec_source).as_posix()}`",
            "",
            "## Coverage",
            "",
    ...
    for finding in findings:
        if (
            finding.audit_status not in {"PASS", "PRESENT_UNREVIEWED"}
            or finding.reference_status == "KO_EC_COMPATIBLE_BUT_BROADER"
        ):
