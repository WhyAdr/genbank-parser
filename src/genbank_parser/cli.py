...
def create_parser() -> argparse.ArgumentParser:
    ...
    subparsers = parser.add_subparsers(
        dest="subcommand", required=True, help="Analysis command to run"
    )

    # 1. validate
    p_val = subparsers.add_parser(
        "validate",
        help="Validate GenBank structure and biological translation semantics",
    )
    ...
    p_meta = subparsers.add_parser(
        "summary",
        aliases=["metadata"],
        help="Extract record metadata and summary stats",
    )
    ...
    # 4. search
    p_srch = subparsers.add_parser(
        "search", help="Search features by gene, product, KO, EC, Pfam, or regex"
    )
    ...
    p_srch.add_argument(
        "--format", choices=["text", "tsv", "csv", "json"], default="text"
    )
    p_srch.add_argument("--output", help="Output file path")

    # 5. locus
    p_loc = subparsers.add_parser(
        "locus", help="Deep-dive single-locus qualifier viewer"
    )
    ...
    p_neigh = subparsers.add_parser(
        "neighborhood", help="Extract genomic neighborhood window around target locus"
    )
    ...
    p_reg = subparsers.add_parser(
        "region", help="Extract genomic sub-region with local feature coordinates"
    )
    ...
    p_reg.add_argument(
        "--rebase",
        action="store_true",
        help="Compatibility flag; coordinates are always local",
    )
    p_reg.add_argument("--output", help="Output file path (.gbk/.fna)")

    # 8. fasta
    p_fa = subparsers.add_parser(
        "fasta", help="Export all CDS translations as protein FASTA"
    )
    ...
    p_seq = subparsers.add_parser(
        "sequence",
        help="Extract genome nucleotide (.fna) and CDS nucleotide (.ffn) sequences",
    )
    ...
    # 10. codon
    p_cod = subparsers.add_parser(
        "codon", help="Calculate codon usage, RSCU, and positional GC metrics"
    )
    ...
    p_cod.add_argument(
        "--include-pseudo", action="store_true", help="Include pseudogenes"
    )

    # 11. functional
    p_func = subparsers.add_parser(
        "functional",
        help="Profile COG distributions and metabolic pathway completeness",
    )
    ...
    p_disc = subparsers.add_parser(
        "discover", help="Scan annotation-supported mobilome or xenobiotic islands"
    )
    ...
    p_comp = subparsers.add_parser(
        "compare", help="Multi-genome marker presence/absence matrix"
    )
    p_comp.add_argument("genomes", nargs="+", help="GenBank files or directories")
    p_comp.add_argument(
        "--targets",
        required=True,
        help="Comma-separated marker names (e.g. 'ladA,ssuD,K20938')",
    )
    p_comp.add_argument("--output", help="Output matrix TSV path")

    # 14. diff
    p_diff = subparsers.add_parser(
        "diff", help="Compare two annotation versions of the same genome"
    )
    ...
    p_phy = subparsers.add_parser(
        "phylo", help="Extract phylogenomic core and housekeeping marker genes"
    )
    p_phy.add_argument("input", help="Input GenBank file")
    p_phy.add_argument(
        "--markers", choices=["core", "housekeeping", "all"], default="all"
    )
    p_phy.add_argument(
        "--min-length",
        type=int,
        default=50,
        help="Minimum candidate translation length in amino acids",
    )
    p_phy.add_argument("--output-dir", help="Output directory for marker FASTAs")

    # 16. crispr
    p_cris = subparsers.add_parser(
        "crispr", help="Detect CRISPR repeat arrays and Cas gene clusters"
    )
    p_cris.add_argument("input", help="Input GenBank file")
    p_cris.add_argument(
        "--window", type=int, default=15000, help="Window for array-Cas linking"
    )

    # 17. gff
    p_gff = subparsers.add_parser(
        "gff", help="Convert GenBank file to standard GFF3 format"
    )
    ...
    # 18. batch-summary
    p_bat = subparsers.add_parser(
        "batch-summary", help="Parse Bakta summaries across multiple isolates"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if cmd == "validate":
        ...
        analyze_functional(
            args.input,
            format_type=args.format,
            pathways_only=args.pathways_only,
